...
(function () {
	...
	var __loadTemplate1 = function (args) {
		...
		var cssToReload = ['aria.widgets.css.' + aria.widgets.AriaSkinInterface.getSkinName() + '.Global'];
		if (cfg.reload) {
	...
	Aria.classDefinition({
		...
		$statics : {
			...
			_startLoad : function (evt, args) {
				...
				Aria.load({
					classes : ['aria.widgets.AriaSkinInterface', 'aria.templates.Layout', 'aria.templates.CfgBeans',
							'aria.utils.Dom', 'aria.templates.TemplateManager'],
					oncomplete : {
						fn : __loadTemplate1,
						args : {
							cfg : cfg,
