...
Aria.classDefinition({
	...
	$dependencies : ["aria.widgets.AriaSkinInterface", "aria.widgets.frames.FrameFactory", "aria.utils.Dom",
			"aria.utils.Function", "aria.widgets.frames.FrameFactory", "aria.utils.Array", "aria.utils.Delegate"],
	/**
	...
	$statics : {
		 ...
		 * TableFrame or FixedHeightFrame) depends on the sprType property of the skin class. If the skin defines icons
	...
	$prototype : {
		...
		_computeIconSize : function (icon, param) {
			...
			var iconParts = stateObject.icons[icon].name.split(":");
			var iconInfo = aria.widgets.AriaSkinInterface.getIcon(iconParts[0], iconParts[1]);
			var active = stateObject.icons[icon].active;
			if (iconInfo) {
		...
		_getIconStyle : function (iconInfo, active) {
			...
			if (this._skinObject.simpleHTML && (aria.core.Browser.isChrome || aria.core.Browser.isSafari)) {
				style.push('margin-top:2px;'); // PTR04877996 icon alignment issue for Chrome and Safari.
			}
			return style.join('');
		},
