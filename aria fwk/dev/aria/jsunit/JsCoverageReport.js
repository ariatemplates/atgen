...
var classDefinition = {
	...
	$prototype : {
		...
		getReport : function (toEmail) {
			var report = "";
			var coverage = parseInt(this.jsCoverage.getCoverage());
			...
			var coverageLink = 
				"<a href='"+ baseUrl +"jscoverage.html' target='_blank'>" +
					"Global Coverage" +
				"</a>";
			
			var markupArray = [
			"<h2>JsCoverage report</h2><br>",
			...
			if (toEmail) {
				markupArray.push(
				"<tr><td width='350px'><strong>"+coverageLink+"</strong> (on " + filesCount + " files and " + linesCount + " lines)</td>", 
					"<td><strong>", coverage, "%<strong></td>",
					'<td width="100px"><table><tbody><tr>',
						'<td bgcolor="lime" width="'+coverage+'px"/>',
						'<td bgcolor="red" width="'+(100-coverage)+'px"/>',
					'</tr></tbody></table></td></tr><br>'
				);
			} else {
				markupArray.push("<tr>",
						"<td><strong>"+coverageLink+"</strong> (on " + filesCount + " files and " + linesCount + " lines)</td>",
						'<td><strong>', coverage, '%</td>',
						'<td style="float: right;margin-right: 5px;">',
							'<div style="background-color: #E00000;border: 1px solid #000000;float: right;height: 10px;margin-top: 4px;overflow: hidden;width: 100px;">',
								'<div style="background-color: #00F000;height: 10px;width: ' + coverage + 'px;"></div>',
							'</div>',
						'</td>',
					"</tr>")
			}
		...
		_createJsCoverageReport : function () {
			...
			_reportContainer.style.cssText = ["background-color: white",
		...
		/**
		 * Make the JsCoverage report container visible 
		 * This method might be called as soon as the test is completed.
		 ...
		 * To be called if anything changed in the JsCoverage instance represented by this reporter (such as filters)
		 */
