...
Aria.classDefinition({
	...
	$css : ['aria.widgets.css.' + aria.widgets.AriaSkinInterface.getSkinName() + '.TextInput'],
	/**
	...
	$prototype : {
		...
		_inputWithFrameMarkup : function (out) {
			var cfg = this._cfg, innerPadding = this._skinObj.innerPadding, hts = this._helpTextSet, htc = this._skinObj.helpText, color = (aria.core.Browser.isIE7)
					? this._skinObj.states[this._state].color
					: "inherit", report;
			var stringUtils = aria.utils.String;

			var inlineStyle = ['padding:', innerPadding.top, 'px ', innerPadding.right, 'px ', innerPadding.bottom,
					'px ', innerPadding.left, 'px;position:relative;'];

			if (!this._simpleHTML) {
				inlineStyle.push("background-color:transparent;border-width:0px;margin:0;vertical-align:top;");
			}
			...
			var inputWidth = this._frame.innerWidth - innerPadding.left - innerPadding.right;
			if (inputWidth < 0) {
				inputWidth = 0;
			}
		...
		_reactToControllerReport : function (report, arg) {
			...
			if (report) {
				...
				if (arg) {
					...
					// default
					if (arg.hasFocus != null) { // replace default hasFocus
						// property
		...
		_updateState : function () {
			...
			var innerPadding = this._skinObj.innerPadding;
			var inputWidth = this._frame.innerWidth - innerPadding.left - innerPadding.right;
			if (inputWidth < 0) {
				inputWidth = 0;
			}
			this.getTextInputField().style.width = inputWidth + "px";
			if (aria.core.Browser.isIE7 && !this._helpTextSet) {
				this.getTextInputField().style.color = this._skinObj.states[this._state].color;
			}
		...
		_dom_onblur : function (event) {
			if (!this._keepFocus) {
				...
				if (this._cfg && this._cfg.prefill) {
					this.setPrefillText(true, this._cfg.prefill, false);
				} else {
					this.setHelpText(true);
				}
				// check if this._cfg exists -> checkValue calls onchange that
				// might dispose the widget
				if (this._cfg) {
					if (!this._cfg.directOnBlurValidation) {
						this.changeProperty("error", false);
					}
		...
		setHelpText : function (enable) {
			...
			var color = enable ? helpTextConfig.color : (aria.core.Browser.isIE7)
					? this._skinObj.states[this._state].color
					: "inherit";
