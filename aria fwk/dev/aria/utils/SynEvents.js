...
(function () {
	...
	var _isInDom = function (element) {
		...
		if (!element || element == window || element == document || !element.parentNode) {
			return false;
		}
	...
	var _fireMouseEnterLeave = function (last, el, options) {
		...
		if (_isWithin(last, el)) {
			...
			Syn.trigger("mouseleave", options, last);
			...
			Syn.trigger("mouseenter", options, el);
	...
	/*
	 * From https://github.com/jupiterjs/syn/, 09.05.2011 FILE synthetic.js
	 */
	...
	steal.then(function () {
		(function () {
			 ...
			 * @test funcunit/synthetic/qunit.html Syn is used to simulate user actions. It creates synthetic events and
			 * performs their default behaviors.
			 * <h2>Basic Use</h2>
			 * The following clicks an input element with <code>id='description'</code> and then types
			 * <code>'Hello World'</code>.
			 * @codestart Syn.click({},'description') .type("Hello World")
			 * @codeend
			 * <h2>User Actions and Events</h2>
			 * <p>
			 * Syn is typically used to simulate user actions as opposed to triggering events. Typing characters is an
			 * example of a user action. The keypress that represents an <code>'a'</code> character being typed is an
			 * example of an event.
			 * </p>
			 * <p>
			 ...
			 * <li><code>[Syn.prototype.dblclick dblclick]</code> - two <code>click!</code> events followed by a
			 * <code>dblclick</code>.</li>
			 ...
			 * <li><code>[Syn.prototype.move move]</code> - moves the mouse from one position to another (triggering
			 * mouseover / mouseouts).</li>
			 * <li><code>[Syn.prototype.drag drag]</code> - a mousedown, followed by mousemoves, and a mouseup.</li>
			 * </ul>
			 * All actions run asynchronously. Click on the links above for more information on how to use the specific
			 * action.
			 * <h2>Asynchronous Callbacks</h2>
			 * Actions don't complete immediately. This is almost entirely because <code>focus()</code> doesn't run
			 * immediately in IE. If you provide a callback function to Syn, it will be called after the action is
			 * completed. <br/>The following checks that "Hello World" was entered correctly:
			 * @codestart Syn.click({},'description') .type("Hello World", function(){ ok("Hello World" ==
			 * document.getElementById('description').value) })
			 * @codeend
			 * <h2>Asynchronous Chaining</h2>
			 * <p>
			 * You might have noticed the [Syn.prototype.then then] method. It provides chaining so you can do a
			 * sequence of events with a single (final) callback.
			 * </p>
			 * <p>
			 ...
			 * @codestart Syn.type('ice water','title') .type('ice and water','description') .click({},'create')
			 * .drag({to: 'favorites'},'newRecipe', function(){ ok($('#newRecipe').parents('#favorites').length); })
			 * @codeend
			 * <h2>jQuery Helper</h2>
			 * If jQuery is present, Syn adds a triggerSyn helper you can use like:
			 * @codestart $("#description").triggerSyn("type","Hello World");
			 * @codeend
			 * <h2>Key Event Recording</h2>
			 * <p>
			 * Every browser has very different rules for dispatching key events. As there is no way to feature detect
			 * how a browser handles key events, synthetic uses a description of how the browser behaves generated by a
			 * recording application.
			 * </p>
			 * <p>
			 * If you want to support a browser not currently supported, you can record that browser's key event
			 * description and add it to <code>Syn.key.browsers</code> by it's navigator agent.
			 * </p>
			 * @codestart Syn.key.browsers["Envjs\ Resig/20070309 PilotFish/1.2.0.10\1.6"] = { 'prevent':
			 * {"keyup":[],"keydown":["char","keypress"],"keypress":["char"]}, 'character': { ... } }
			 * @codeend
			 * <h2>Limitations</h2>
			 * Syn fully supports IE 6+, FF 3+, Chrome, Safari, Opera 10+. With FF 1+, drag / move events are only
			 * partially supported. They will not trigger mouseover / mouseout events.<br/> Safari crashes when a
			 * mousedown is triggered on a select. Syn will not create this event.
			 * <h2>Contributing to Syn</h2>
			 * Have we missed something? We happily accept patches. The following are important objects and properties
			 * of Syn:
			 * <ul>
			 * <li><code>Syn.create</code> - contains methods to setup, convert options, and create an event of a
			 * specific type.</li>
			 ...
			 * <p>
			 * Syn is really the foundation of JavaScriptMVC's functional testing framework - [FuncUnit]. But, we've
			 * purposely made Syn work without any dependencies in the hopes that other frameworks or testing solutions
			 * can use it as well.
			 * </p>
			 * @constructor Creates a synthetic event on the element.
			 * @param {Object} type
			...
			extend(Syn, {
				...
				/**
				 * @attribute defaults Default actions for events. Each default function is called with this as its
				 * element. It should return true if a timeout should happen after it. If it returns an element, a
				 * timeout will happen and the next event will happen on that element.
				 */
				defaults : {
					focus : function () {
				...
				isFocusable : function (elem) {
					var attributeNode;
					return (this.focusable.test(elem.nodeName) || ((attributeNode = elem.getAttributeNode("tabIndex")) && attributeNode.specified))
							&& Syn.isVisible(elem);
				},
				/**
				...
				helpers : {
					...
					addOffset : function (options, el) {
						...
						if (typeof options === 'object' && options.clientX === undefined
								&& options.clientY === undefined && options.pageX === undefined
				...
				/**
				 * @attribute
				 * @hide An object of eventType -> function that create that event.
				 */
				create : {
					// -------- PAGE EVENTS ---------------------
					page : {
						event : function (type, options, element) {
							var doc = Syn.helpers.getWindow(element).document || Aria.$window.document, event;
							if (doc.createEvent) {
								event = doc.createEvent("Events");
				...
				/**
				 * @attribute support Feature detected properties of a browser's event system. Support has the following
				 * properties:
				 * <ul>
				...
				/**
				 * Creates a synthetic event and dispatches it on the element. This will run any default actions for the
				 * element. Typically you want to use Syn, but if you want the return value, use this.
				 * @param {String} type
				 * @param {Object} options
				...
				trigger : function (type, options, element) {
					...
					var create = Syn.create, setup = create[type] && create[type].setup, kind = key.test(type)
							? 'key'
							: (page.test(type) ? "page" : "mouse"), createType = create[type] || {}, createKind = create[kind], event, ret, autoPrevent, dispatchEl = element;
					...
					ret && Syn.support.ready === 2 && Syn.defaults[type]
			...
			extend(Syn.init.prototype, {
				 ...
				 * @function then
				 * <p>
				 * Then is used to chain a sequence of actions to be run one after the other. This is useful when many
				 * asynchronous actions need to be performed before some final check needs to be made.
				 * </p>
				 * <p>
				 * The following clicks and types into the <code>id='age'</code> element and then checks that only
				 * numeric characters can be entered.
				 * </p>
				 * <h3>Example</h3>
				 * @codestart Syn('click',{},'age') .then('type','I am 12',function(){ equals($('#age').val(),"12") })
				 * @codeend If the element argument is undefined, then the last element is used.
				 ...
				 * @param {String|HTMLElement} [element] A element's id or an element. If undefined, defaults to the
				 * previous element.
				 * @param {Function} [callback] A function to callback after the action has run, but before any future
				 * chained actions are run.
				 */
				...
				/**
				 * @function click Clicks an element by triggering a mousedown, mouseup, and a click event.
				 * <h3>Example</h3>
				 * @codestart Syn.click({},'create',function(){ //check something })
				 * @codeend You can also provide the coordinates of the click. If jQuery is present, it will set clientX
				 * and clientY for you. Here's how to set it yourself:
				 * @codestart Syn.click( {clientX: 20, clientY: 100}, 'create', function(){ //check something })
				 * @codeend You can also provide pageX and pageY and Syn will convert it for you.
				 * @param {Object} options
				 * @param {HTMLElement} element
				 * @param {Function} callback
				 */
				...
				/**
				 * @function dblclick Dblclicks an element. This runs two [Syn.prototype.click click] events followed by
				 * a dblclick on the element.
				 * <h3>Example</h3>
				 * @codestart Syn.dblclick({},'open')
				 * @codeend
				 * @param {Object} options
				 * @param {HTMLElement} element
				 * @param {Function} callback
			...
			var actions = ["click", "dblclick", "move", "drag", "key", "type", 'rightClick'], makeAction = function (
					name) {
			 ...
			 * @codestart new MVC.Syn('click').send(MVC.$E('id'))
			 * @codeend
			 * @constructor Sets up a synthetic event.
			 * @param {String} type type of event, ex: 'click'
			 * @param {optional:Object} options
			 */
			if (Aria.$window.jQuery || (Aria.$window.FuncUnit && Aria.$window.FuncUnit.jquery)) {
				((Aria.$window.FuncUnit && Aria.$window.FuncUnit.jquery) || Aria.$window.jQuery).fn.triggerSyn = function (
	...
	/*
	 * From https://github.com/jupiterjs/syn/, 09.05.2011 FILE mouse.js
	 */
		...
		(function () {
			...
			h.extend(Syn.defaults, {
				...
				click : function () {
					...
					// submit a form
					if (!(Syn.support.clickSubmits) && (nodeName == "input" && element.type == "submit")
							|| nodeName == 'button') {
			...
			h.extend(Syn.create, {
				mouse : {
					options : function (type, options, element) {
						var doc = Aria.$window.document.documentElement, body = Aria.$window.document.body, center = [
								options.pageX || 0, options.pageY || 0],
						// browser might not be loaded yet (doing support code)
						left = Syn.mouse.browser && Syn.mouse.browser.left[type], right = Syn.mouse.browser
								&& Syn.mouse.browser.right[type];
						return h.extend({
							bubbles : true,
							cancelable : true,
							view : Aria.$window,
							...
							screenX : 1,
							screenY : 1,
							clientX : options.clientX || center[0]
									- (doc && doc.scrollLeft || body && body.scrollLeft || 0) - (doc.clientLeft || 0),
							clientY : options.clientY || center[1]
									- (doc && doc.scrollTop || body && body.scrollTop || 0) - (doc.clientTop || 0),
							ctrlKey : !!Syn.key.ctrlKey,
							altKey : !!Syn.key.altKey,
							shiftKey : !!Syn.key.shiftKey,
							metaKey : !!Syn.key.metaKey,
							button : left && left.button != null ? left.button : right && right.button
									|| (type == 'contextmenu' ? 2 : 0),
	...
	// do support code
			(function () {
				...
				div.innerHTML = "<form id='outer'>" + "<input name='checkbox' type='checkbox'/>"
						+ "<input name='radio' type='radio' />" + "<input type='submit' name='submitter'/>"
						+ "<input type='input' name='inputter'/>" + "<input name='one'>" + "<input name='two'/>"
						+ "<a href='javascript:__synthTest()' id='synlink'></a>" + "<select><option></option></select>"
						+ "</form>";
	...
	/*
	 * From https://github.com/jupiterjs/syn/, 09.05.2011 FILE browser.js
	 */
	...
	/*
	 * From https://github.com/jupiterjs/syn/, 09.05.2011 FILE key.js
	 */
		...
		(function () {
			...
			getSelection = function (el) {
				...
				if (el.selectionStart !== undefined) {
					...
					if (Aria.$window.document.activeElement && Aria.$window.document.activeElement != el
			...
			h.extend(Syn, {
				/**
				 * @attribute A list of the keys and their keycodes codes you can type. You can add type keys with
				 * @codestart Syn('key','delete','title'); //or Syn('type','One Two Three[left][left][delete]','title')
				 * @codeend The following are a list of keys you can type:
				 * @codestart text \b - backspace \t - tab \r - enter ' ' - space a-Z 0-9 - normal characters /!@#$*,.? -
				 * All other typeable characters page-up - scrolls up page-down - scrolls down end - scrolls to bottom
				 * home - scrolls to top insert - changes how keys are entered delete - deletes the next character left -
				 * moves cursor left right - moves cursor right up - moves the cursor up down - moves the cursor down
				 * f1-12 - function buttons shift, ctrl, alt - special keys pause-break - the pause button scroll-lock -
				 * locks scrolling caps - makes caps escape - escape button num-lock - allows numbers on keypad print -
				 * screen capture
				 * @codeend
				 */
				keycodes : {
			...
			h.extend(Syn.key, {
				...
				/**
				 * @hide gets the options for a key and event type ...
				 * @param {Object} key
				 * @param {Object} event
				 */
				options : function (key, event) {
					...
					// string keycode ?
					result.keyCode = parseInt(result.keyCode, 10);
				...
				kinds : {
					...
					specialChars : ["\b"],
					navigation : ["page-up", 'page-down', 'end', 'home', 'left', 'up', 'right', 'down', 'insert',
							'delete'],
				...
				defaults : {
					...
					'\t' : function (options, scope) {
						...
						elIndex, firstNotIndexed, prev, orders = [];
						for (; i < focusEls.length; i++) {
							orders.push([focusEls[i], i]);
						}
						var sort = function (order1, order2) {
							var el1 = order1[0], el2 = order2[0], tab1 = Syn.tabIndex(el1) || 0, tab2 = Syn.tabIndex(el2)
									|| 0;
							if (tab1 == tab2) {
					...
					'right' : function (options, scope, key, force, sel) {
						if (Syn.typeable.test(this.nodeName)) {
							if (Syn.key.shiftKey) {
								Syn.selectText(this, sel.start, sel.end + 1 > this.value.length
										? this.value.length
										: sel.end + 1)
							} else {
			...
			h.extend(Syn.init.prototype, {
				/**
				 * @function key Types a single key. The key should be a string that matches a [Syn.static.keycodes].
				 * The following sends a carridge return to the 'name' element.
				 * @codestart Syn.key('\r','name')
				 * @codeend For each character, a keydown, keypress, and keyup is triggered if appropriate.
				...
				_key : function (options, element, callback) {
					...
					var caret = Syn.typeable.test(element.nodeName) && getSelection(element), key = convert[options]
							|| options,
				...
				/**
				 * @function type Types sequence of [Syn.key key actions]. Each character is typed, one at a type.
				 * Multi-character keys like 'left' should be enclosed in square brackents. The following types
				 * 'JavaScript MVC' then deletes the space.
				 * @codestart Syn.type('JavaScript MVC[left][left][left]\b','name')
				 * @codeend Type is able to handle (and move with) tabs (\t). The following simulates tabing and
				 * entering values in a form and eventually submitting the form.
				 * @codestart Syn.type("Justin\tMeyer\t27\tjustinbmeyer@gmail.com\r")
				 * @codeend
				...
				_type : function (options, element, callback) {
					...
					var parts = options.match(/(\[[^\]]+\])|([^\[])/g), self = this, runNextPart = function (
							runDefaults, el) {
	...
	/*
	 * From https://github.com/jupiterjs/syn/, 09.05.2011 FILE drag/drag.js
	 */
	(function () {
		...
		startMove = function (start, end, duration, element, callback) {
			var startTime = new Date(), distX = end.clientX - start.clientX, distY = end.clientY - start.clientY, win = Syn.helpers.getWindow(element), current = elementFromPoint(start, element), cursor = win.document.createElement('div'), calls = 0;
			var move = function () {
				// get what fraction we are at
				var now = new Date(), scrollOffset = Syn.helpers.scrollOffset(win), fraction = (calls == 0 ? 0 : now
						- startTime)
						/ duration, options = {
		...
		adjust = function (from, to, win) {
			if (from.clientY < 0) {
				var off = Syn.helpers.scrollOffset(win);
				var dimensions = Syn.helpers.scrollDimensions(win), top = off.top + (from.clientY) - 100, diff = top
						- off.top
		...
		Syn.helpers.extend(Syn.init.prototype, {
			/**
			 * @function move Moves the cursor from one point to another. ### Quick Example The following moves the
			 * cursor from (0,0) in the window to (100,100) in 1 second. Syn.move( { from: {clientX: 0, clientY: 0}, to:
			 * {clientX: 100, clientY: 100}, duration: 1000 }, document.document) ## Options There are many ways to
			 * configure the endpoints of the move. ### PageX and PageY If you pass pageX or pageY, these will get
			 * converted to client coordinates. Syn.move( { from: {pageX: 0, pageY: 0}, to: {pageX: 100, pageY: 100} },
			 * document.document) ### String Coordinates You can set the pageX and pageY as strings like: Syn.move( {
			 * from: "0x0", to: "100x100" }, document.document) ### Element Coordinates If jQuery is present, you can
			 * pass an element as the from or to option and the coordinate will be set as the center of the element.
			 * Syn.move( { from: $(".recipe")[0], to: $("#trash")[0] }, document.document) ### Query Strings If jQuery
			 * is present, you can pass a query string as the from or to option. Syn.move( { from: ".recipe", to:
			 * "#trash" }, document.document) ### No From If you don't provide a from, the element argument passed to
			 * Syn is used. Syn.move( { to: "#trash" }, 'myrecipe') ### Relative You can move the drag relative to the
			 * center of the from element. Syn.move("+20 +30", "myrecipe");
			...
			_move : function (options, from, callback) {
				...
				var win = Syn.helpers.getWindow(from), fro = convertOption(options.from || from, win, from), to = convertOption(options.to
						|| options, win, from);
			...
			/**
			 * @function drag Creates a mousedown and drags from one point to another. Check out [Syn.prototype.move
			 * move] for API details.
			 * @param {Object} options
			 * @param {Object} from
			 * @param {Object} callback
			 */
