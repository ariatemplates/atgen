...
(function () {
	...
	Aria.classDefinition({
		...
		$constructor : function (cfg, context, lineNumber) {
			...
			this.$assert(18, context && context.createId);
			...
			/**
			 * Map of bindings for listeners, used when listeners have to be removed
			 * @type Array
			 * @protected
			 */
						...
						if (inside && (to || to === 0)) {
							...
							if (typeof(bindValue) != "undefined") {
								if (transform) {
									...
									bindValue = this._transformToWidget(transform, bindValue);
									if (typeof bindValue == "undefined") {
								...
								if (typeof(this._cfg[bindedProperty]) != "undefined") {
									var valueToDataModel = this._transformFromWidget(transform, this._cfg[bindedProperty]);
			...
			if (!cfg.id) {
				...
				this._domId = this._context.createId(cfg.id);
			}
		},
		$destructor : function () {
			...
			if (this._cfg) {
				...
				if (bnds != null) {
					...
					for (var key in bnds) {
						if (bnds.hasOwnProperty(key) && bnds[key] && this._bindingListeners[key]) {
							json.removeListener(bnds[key].inside, bnds[key].to, this._bindingListeners[key]);
							delete this._bindingListeners[key];
						}
					}
				}
				this._cfg = null;
		...
		$prototype : {
				...
				if (bnds) {
					for (var key in bnds) {
						...
						if (bnd) {
							...
							// save it for disposal
							this._bindingListeners[key] = jsonChangeCallback;

							try {
								jsonUtils.addListener(bnd.inside, bnd.to, jsonChangeCallback, true);
							} catch (e) {
								this.$logError(this.WIDGET_BINDING_ERROR, [bnd.inside, bnd.to]);
							}

							// Between the moment the widget constructor is called, and the moment this
							// method is called, some time may have elapsed (especially for template-based widgets),
							// and binded values may have changed.
							var newValue = this._transformToWidget(bnd.transform, bnd.inside[bnd.to]);
							this.setWidgetProperty(key, newValue);
						}
					}
				}
			},
			...
			setProperty : function (propertyName, newValue) {
				...
				if (bnds) {
					...
					if (target) {
						...
						if (holder && (nm || nm === 0)) {
							// If we have a transform specified,
							// transform the widget value to data model value before storing in data model
							structureValue = this._transformFromWidget(transform, newValue);
							jsonUtils.setValue(holder, nm, structureValue, this._bindingListeners[propertyName]);
						}
					}
				}
			...
			_notifyDataChange : function (args, propertyName) {
				...
				var bnd = this._cfg.bind[propertyName], newValue = this._transformToWidget(bnd.transform, bnd.inside[bnd.to]);
			...
			/**
			 * Transforms a value from the data model value to the corresponding widget value using the specified
			 * transform.
			 * @protected
			 * @param {Multitype} transform a transform bean specifying the transformation. Can be undefined if no
			 * transformation is to be used.
			 * @param {Multitype} value The data model value to transform to widget value.
			 * @return {Multitype} The transformed value (widget value). If no transformation is specified, returns the
			 * same value as passed in the value parameter.
			 */
			_transformToWidget : function (transform, value) {
				var retVal = value;
				if (transform) {
					var created = false;
					...
					if (transform.toWidget) {
						retVal = this.evalCallback(transform.toWidget, retVal);
			 ...
			 * @param {Multitype} transform a transform bean specifying the transformation. Can be undefined if no
			 * transformation is to be used.
			 * @param {Multitype} value The widget value to transform to data model value.
			 * @return {Multitype} The transformed value (data model value). If no transformation is specified, returns
			 * the same value as passed in the value parameter.
			 */
			_transformFromWidget : function (transform, value) {
				var retVal = value;
				if (transform) {
					var created = false;
					// Instantiate the class if we refer to a class path
					...
					if (transform.fromWidget) {
						retVal = this.evalCallback(transform.fromWidget, retVal);
					} else if (typeof(transform) == 'function') {
						retVal = this.evalCallback(transform, retVal);
					}
					if (created) {
			...
			/**
			 * Call a callback function of the widget. First parameter to callback function will be the arg parameter
			 * and the second parameter will be the callback.args if specified.
			 * @param callback A json callback object specifying the callback
			 * @param arg Any parameter which will be passed as first parameter to callback function.
			 */
			evalCallback : function (callback, arg) {
				return this._context.evalCallback(callback, arg);
			},
