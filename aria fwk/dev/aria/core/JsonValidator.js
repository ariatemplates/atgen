...
(function () {
	...
	/**
	 * The JSON Validator does two main operations: - a preprocessing operation is done when loading a bean package (BP)
	 * definition During this operation, every bean definition in the package is annotated, so that it contains a
	 * reference to the bean definition of its immediate super bean and its base built-in type, and bean inheritance is
	 * processed (propagation of object properties of a bean to the beans which extend it). After the preprocessing of a
	 * bean, its default value, if provided, is checked so that it matches the definition. - the processing is done when
	 * validating an instance of a JSON object against the bean definition it is supposed to comply with. Note: this
	 * class is tightly linked with JsonTypesCheck, to keep files with a reasonnable size Be carefull if changing
	 * something: any protected method in this class (method whose name starts with one underscore) may be called from
	 * JsonTypesCheck. Private methods (starting with two underscores) are not called from JsonTypesCheck.
	 * @class aria.core.JsonValidator
	...
	Aria.classDefinition({
		...
		$prototype : {
			...
			_preprocessBean : function (beanDef, beanName, packageDef) {
				...
				if (hasNoDefault && "$default" in typeRef) {
