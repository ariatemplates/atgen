...
Aria.classDefinition({
	...
	$constructor : function () {
		...
		// TO BE FIXED LATER
		// PTR 04893174: do not set this._rootElement = document.body on Firefox, as it resets all scrolling
		// position when changing the overflow style
		// PTR 05210073: the changes reverted for PTR 04893174 were put back in place and the code changes in order to
	...
	$prototype : {
		...
		_getComputedStyle : function () {
			...
			if (this.conf.preferredWidth > 0) {
				...
				// PTR 04315046: calling aria.utils.Size.getFreeSize(this.domElement) is not possible here as it moves
		...
		_getPosition : function (size) {
			...
			if (this.conf.center) {
				...
				var i = 0, isInViewSet, preferredPosition;
				do {
					...
					// If this position+size is out of the viewport, try the next anchor available
		...
		_getPositionForAnchor : function (preferredPosition, size) {
			...
			// Depending on the popup's anchor configuration, the origin can be modified as well
			// Anchor at the bottom of the popup, substract the height of the popup to the top
			...
			// Anchor at the right of the popup, substract the width of the popup to the top
		...
		/**
		 * Check that a given anchor only ahs valid values ('bottom', 'top', 'left', 'right') And has a reglementary
		 * size
		 * @param {String} anchor
		 * @return {Boolean}
		 * @protected
		...
		_show : function () {
			...
			if (this.modalMaskDomElement) {
				...
				// Compute the style after scrollbars are removed from the document. Thus the dialog can be properly
				// centered
		...
		closeOnMouseOut : function (domEvent) {
			if (this.conf.closeOnMouseOut) {
				if (!this.conf.closeOnMouseOutDelay) { // If delay is not set OR is equal to zero
					this.close(domEvent);
				} else {
		...
		setSection : function (section) {
			// PROFILING // var profilingId = this.$startMeasure("Inserting section in DOM");
