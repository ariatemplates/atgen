...
Aria.classDefinition({
	...
	$singleton : true,
	$dependencies : ["aria.utils.String", "aria.utils.Type", "aria.utils.environment.Date"],
	$resources : {
	...
	$constructor : function () {
		 ...
		 * @type Number
		 */
		this.firstDayOfWeek = res.firstDayOfWeek;
	...
	$prototype : {
		 ...
		 * @param {String} entryStr
		 * @param {Date} referenceDate This optional reference date will be used, if supplied, to calculate the +/-N
		 * expression
		 * @return {Date}
		 */
		interpret : function (entryStr, referenceDate) {

			var dateBeforeMonth, jsdate, dateArray, interpretdDate, interpretdMonth, interpretdYear, entry;
			...
			if (this._interpret_specialCase3.test(entry)) {
				var execResult = this._interpret_specialCase3.exec(entry);
				...
				jsdate = this.interpret(newEntry);
				if (jsdate) {
					jsdate.setDate(jsdate.getDate() + shift);
				}
				return jsdate;
			...
			if (entry.length == 3) {
				...
				if (this._interpret_isMonth.test(entry)) {
					...
					for (var i = 0; i < 12; i++) {
						if (entry === this._interpret_monthTexts[i]) {
							jsdate = new Date();
							if (jsdate.getMonth() != i) {
								jsdate.setDate(1);
								jsdate.setMonth(i);
							}
							return jsdate;
			 ...
			 * interpret it
			 */
			this.$assert("234", entry.length > 3);

			dateBeforeMonth = this._environment.getDateFormats().dateBeforeMonth;

			// in 010210 format, we are sure that we can cut at index 2 and
			// 4
			...
			if (this._interpret_digitOnly.test(interpretdMonth)) {
				...
				for (var j = 0; j < 12; j++) {
					if (interpretdMonth === this._interpret_monthTexts[j]) {
						interpretdMonth = j;
						break;
					}
				}
			...
			if (interpretdYear) {
				...
				jsdate = new Date();
				var todayTime = new Date(jsdate.getFullYear(), jsdate.getMonth(), jsdate.getDate()).getTime();

				jsdate = new Date(jsdate.getFullYear(), interpretdMonth, interpretdDate);

				// if date is before today
				if (jsdate.getTime() < todayTime) {
					jsdate = new Date(jsdate.getFullYear() + 1, interpretdMonth, interpretdDate);
				}

				// check if date was valid (Date interpretd unvalid
				// entries, but transform them)
				if (jsdate.getDate() != interpretdDate || jsdate.getMonth() != interpretdMonth) {
					return null;
				} else {
					return jsdate;
		 ...
		 * @param {Date} date
		 * @param {String} pattern. See http://topspot/index.php/Aria_Templates_Dates
		...
		_pushIfExists : function (fnParts, patternChar, patternOccurency, originalPattern) {
			...
			if (patternOccurency) {
				...
				if (patternChar == 'U') {
					return this._formatToUpperCase = true;
				}
