...
Aria.classDefinition({
	...
	$css : ["aria.widgets.css." + aria.widgets.AriaSkinInterface.getSkinName() + ".Div"],
	/**
	 * Div constructor
	...
	$prototype : {
		 ...
		 * @param {aria.widgets.CfgBeans.ActionWidgetCfg} cfg the widget configuration (only width, height, maxWidth, maxHeight will be used)
		 */
		updateSize : function (cfg) {
			...
			if (hasChanged) {
				/*
				// Change width and height if it's too large compared to the viewport
				var viewport = aria.utils.Dom._getViewportSize();
				if (viewport.width < this._cfg.width) {
					this._cfg.width = viewport.width;
				}
				if (viewport.height < this._cfg.height) {
					this._cfg.height = viewport.height;
				}
				*/
