...
(function () {
	...
	/**
	 * The list widget is constructed with a sub module controller and a sub template. This is the sub module
	 * controller.
	 * @class aria.widgets.form.list.ListController
	 */
	Aria.classDefinition({
		...
		$destructor : function () {
			...
			this._setItems(null); // remove listeners
		...
		$prototype : {
			...
			/**
			 * Allow automatic updates in the data model (for example: selectedCount) when selecting or unselecting an
			 * item in the list. (to be used in conjunction with _stopUpdates).
			 * @protected
			 */
			 ...
			 * @param {Array} itemsList array of items (sub-array of this._data.items) whose currentlyDisabled
			...
			/**
			 * Called when the selected property of an item changed. This method updates the selectedCount and
			 * selectedIndex properties in the data model and unselect other items if the maximum number of selected
			 ...
			 * maxSelectedCount properties (multipleSelect takes precedence). If there is no maximum number, return a
			 * number that is higher than the current number of items in the list.
			 * @return {Number}
			 * @protected
			 */
			 ...
			 * with label/value pairs, or a specification of a container and where to get the label and value properties
			 * in that container.
			...
			_mergeItemsAndSelectionInfo : function (items, selectedValues, selectedIdx) {
				...
				for (var item = 0, length = container.length; item < length; item++) {
					...
					var itemSelected = selectedValues
							? aria.utils.Array.contains(selectedValues, itemObj[valueProperty]) || selectedIdx === item
							: false;
				...
				if (numberOfColumns == 1) {
					...
					if (keyCode == aria.DomEvent.KC_ARROW_UP) {
						...
						if (flowOrientation == 'vertical') {
							/*
							 * if (!(focusIndex % numberOfRows == 0)) { moveFocus = focusIndex - 1; } else { return
							 * focusIndex; }
							 */
						...
						if (flowOrientation == 'vertical') {
							/*
							 * if (!((focusIndex + 1) % numberOfRows == 0)) { moveFocus = focusIndex + 1; } else {
							 * return focusIndex; }
							 */
							moveFocus = focusIndex + 1;
							...
							// Horizontal
							if (!((numberItems - focusIndex) <= numberOfColumns)) {
								moveFocus = focusIndex + numberOfColumns;
							} else {
								return focusIndex;
							}
						...
						if (flowOrientation == 'horizontal') {
							/*
							 * if (!(focusIndex % numberOfColumns == 0)) { moveFocus = focusIndex - 1; } else { return
							 * focusIndex; }
							 */
							moveFocus = focusIndex - 1;
						...
						if (flowOrientation == 'horizontal') {
							moveFocus = focusIndex + 1;
							/*
							 * if (!((focusIndex + 1) % numberOfColumns == 0)) { moveFocus = focusIndex + 1; } else {
							 * return focusIndex; }
							 */
						} else {
							// verticle

							if (!(numberItems - focusIndex <= numberOfRows)) {
								moveFocus = focusIndex + numberOfRows;
							} else {
								return focusIndex;
							}
			...
			/**
			 * Notify the list controller that a key has been pressed. The controller reacts by sending a keyevent
			...
			setFocusedIndex : function (newValue) {
				...
				if (newValue == null) {
					...
					// James added newValue === -1 because when we want to press up on the multi-select
					// and the index is at the first Item we want to close the list and return focus to the text input
					return;
				}
				...
				if (newValue < 0) {
					...
					/*
					 * Int this.json.setValue(data, "focusIndex", data.itemsView.items[newValue].value.index);
					 * this.setFocus(data.focusIndex); return true;
					 */
			 ...
			 * @param {Boolean} alreadyChanged if true the selected property of the item was already changed (so that it
			 * is not necessary to change it again, only raise the itemClick event)
			 */
			...
			/**
			 * Selects all values in order that they are stored in the datamodel. If any values are initially disabled
			 * they will not be selected. If a value becomes disabled due to the maxOptions constraint then they will
			 * not be selected.
			 */
			selectAll : function () {
				var selected = [];
