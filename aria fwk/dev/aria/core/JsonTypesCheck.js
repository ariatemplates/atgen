...
(function () {
	...
	var __badTypeError = function (baseType, args) {
		jv._logError(jv.INVALID_TYPE_VALUE, [baseType.typeName, args.beanDef[jv._MD_TYPENAME], args.value,
				args.path]);
	};
	...
	var baseTypes = [{
...

			{
				...
				makeFastNorm : function (beanDef) {
					...
					// write fast normalizer
					// var genFun;
					// eval("genFun = function(obj) {" + strBuffer.join("\n") + "};");
					// beanDef.$fastNorm = genFun;
				...
				makeFastNorm : function (beanDef) {
					...
					beanDef.$fastNorm = new Function("obj", "if (!obj) { return this.$getDefault(); } for (var i = 0, l = obj.length;"
							+ " i < l; i++) { this.$contentType.$fastNorm(obj[i]); } return obj;");
				}
			}, {
				typeName : "Map",
				dontSkip : true,
				...
				makeFastNorm : function (beanDef) {
					...
					beanDef.$fastNorm = new Function("obj", "if (!obj) { return this.$getDefault(); } for (var key in obj) {"
							+ " if (obj.hasOwnProperty(key)) { this.$contentType.$fastNorm(obj[key]); } } return obj;");
				}
			}, {
				typeName : "MultiTypes",
				preprocess : function (beanDef, beanName, packageDef) {
	...
	/**
	 * @class aria.core.JsonTypesCheck Contains the definition of base types used by the JsonValidator.
