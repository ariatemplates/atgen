...
Aria.classDefinition({
	...
	$dependencies : ["aria.modules.queuing.SimpleSessionQueuing", "aria.modules.RequestBeans",
			"aria.modules.environment.Environment", "aria.modules.urlService.environment.UrlService",
	...
	$constructor : function () {
		 ...
		 * @type Object
		 */
		this.jsonUtils = aria.utils.Json;
		 ...
		 * @protected
		 * @type aria.modules.environment.EnvironmentCfgBeans.RequestJsonSerializerCfg
		 */
		this._requestJsonSerializer = aria.modules.environment.Environment.getRequestJsonSerializerCfg();
	...
	$prototype : {
		...
		_callAsyncRequest : function (args) {
			...
			// determine post data if not set by filters
			if (req.data == null && req.method == "POST") {
				// we encode POST parameters
				req.data = 'data=' + this.serializeRequestData(req.jsonData, req.requestObject);
			}
		...
		_processOnResponse : function (args) {
			...
			var handler = args.requestHandler || this.__getRequestHandler();
		 ...
		 */
		serializeRequestData : function (data, requestObject) {
			...
			if (instance) {
				return this.jsonUtils.convertToJsonString(data, this.jsonUtils.copy(options, true), instance);
			} else {
