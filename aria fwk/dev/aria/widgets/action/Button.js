...
Aria.classDefinition({
	...
	$css : ["aria.widgets.css." + aria.widgets.AriaSkinInterface.getSkinName() + ".Button"],
	/**
	...
	$prototype : {
		...
		_updateState : function (skipChangeState) {
			...
			if (!skipChangeState) {
				...
				if (this.simpleHTML) {
					if (state == "disabled") {
						this._frame.domElt.setAttribute("disabled", "disabled");
					}
				}
			}
		},
		...
		_widgetMarkup : function (out) {
			...
			if (this._simpleHTML) {
				...
				if (ie67) {
					...
					out.write(['<span class="xButton" style="margin: 0;"', tabIndexString, ariaTestMode, '>'].join(''));
				} else {
					out.write(['<button type="button" class="xButton"', tabIndexString, ariaTestMode, '>'].join(''));
				}
