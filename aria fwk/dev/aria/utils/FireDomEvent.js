...
(function () {
	...
	/*
	 * Note: Intentionally not for YUIDoc generation. Simulates a mouse event using the given event information to
	 * populate the generated event object. This method does browser-equalizing calculations to account for differences
	 * in the DOM and IE event models as well as different browser quirks. @method simulateMouseEvent @private @static
	 * @param {HTMLElement} target The target of the given event. @param {String} type The type of event to fire. This
	 * can be any one of the following: click, dblclick, mousedown, mouseup, mouseout, mouseover, and mousemove. @param
	 * {Boolean} bubbles (Optional) Indicates if the event can be bubbled up. DOM Level 2 specifies that all mouse
	 * events bubble by default. The default is true. @param {Boolean} cancelable (Optional) Indicates if the event can
	 * be canceled using preventDefault(). DOM Level 2 specifies that all mouse events except mousemove can be
	 * cancelled. The default is true for all events except mousemove, for which the default is false. @param {Window}
	 * view (Optional) The view containing the target. This is typically the window object. The default is window.
	 * @param {int} detail (Optional) The number of times the mouse button has been used. The default value is 1. @param
	 * {int} screenX (Optional) The x-coordinate on the screen at which point the event occured. The default is 0.
	 * @param {int} screenY (Optional) The y-coordinate on the screen at which point the event occured. The default is
	 * 0. @param {int} clientX (Optional) The x-coordinate on the client at which point the event occured. The default
	 * is 0. @param {int} clientY (Optional) The y-coordinate on the client at which point the event occured. The
	 * default is 0. @param {Boolean} ctrlKey (Optional) Indicates if one of the CTRL keys is pressed while the event is
	 * firing. The default is false. @param {Boolean} altKey (Optional) Indicates if one of the ALT keys is pressed
	 * while the event is firing. The default is false. @param {Boolean} shiftKey (Optional) Indicates if one of the
	 * SHIFT keys is pressed while the event is firing. The default is false. @param {Boolean} metaKey (Optional)
	 * Indicates if one of the META keys is pressed while the event is firing. The default is false. @param {int} button
	 * (Optional) The button being pressed while the event is executing. The value should be 0 for the primary mouse
	 * button (typically the left button), 1 for the terciary mouse button (typically the middle button), and 2 for the
	 * secondary mouse button (typically the right button). The default is 0. @param {HTMLElement} relatedTarget
	 * (Optional) For mouseout events, this is the element that the mouse has moved to. For mouseover events, this is
	 * the element that the mouse has moved from. This argument is ignored for all other events. The default is null.
	 */
	function simulateMouseEvent (target /* :HTMLElement */, type /* :String */, bubbles /* :Boolean */,
			...
			relatedTarget /* :HTMLElement */) /* :Void */
	{
		...
		// check target
		if (!target) {
			fireDomEvent.$logError("simulateMouseEvent(): Invalid target.");
		}
		...
		if (typeUtils.isString(type)) {
			...
			if (!mouseEvents[type]) {
				fireDomEvent.$logError("simulateMouseEvent(): Event type '" + type + "' not supported.");
			}
		} else {
			fireDomEvent.$logError("simulateMouseEvent(): Event type must be a string.");
		}
		...
		if (typeUtils.isFunction(document.createEvent)) {
			...
			fireDomEvent.$logError("simulateMouseEvent(): No event simulation framework present.");
		}
	}
