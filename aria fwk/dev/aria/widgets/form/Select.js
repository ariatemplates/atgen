...
Aria.classDefinition({
	...
	$css : ["aria.widgets.css." + aria.widgets.AriaSkinInterface.getSkinName() + ".Select",
	...
	$prototype : {
		...
		/**
		 * Checks if the value has been changed and if it has then calls the onchange callback for the widget.
		 * @param {Boolean} hasChanged If true calls the onchange callback
		 */
		_hasChanged : function (hasChanged) {
			if (hasChanged != null && this._cfg.onchange) {
		...
		_updateValue : function (includeController) {
			if (this._skinObj.simpleHTML) {
				var hasChanged = this.setProperty("value", this.getSelectField().value);
				this._hasChanged(hasChanged);
			} else if (includeController) {
				var controller = this.controller;
				var dataModel = controller.getDataModel();
				var hasChanged = this.setProperty("value", dataModel.value);
				if (hasChanged && this._cfg) {
