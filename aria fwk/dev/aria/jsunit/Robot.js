 ...
 */
/**
 * Wrapper for the RobotApplet to generate mouse and keyboard low level inputs and screen captures. (This class is still
 * experimental currently, and is marked private for that reason)
 * @class aria.jsunit.Robot
 * @private
 */
	...
	$prototype : {
		...
		initRobot : function (cb) {
			if (this._applet == null) {
				...
				var uri = aria.core.DownloadMgr.resolveURL('aria/jsunit/RobotApplet.jar');
				var applet = document.createElement("applet");
				var param = document.createElement("param");
				param.setAttribute("name", "initCallback");
				param.setAttribute("value", this.INIT_CALLBACK_NAME + "();");
				...
				applet.setAttribute("archive", uri);
		...
		absoluteMouseMove : function (position, cb) {
			this._applet.absoluteMouseMove(position.x, position.y);
			this._callCallback(cb);
		},
		...
		_mouseMoveCb : function (unused, args) {
			...
			this._applet.mouseMove(position.x, position.y);
			this._callCallback(args.cb);
		},

		mousePress : function (buttons, cb) {
			this._applet.mousePress(buttons);
			this._callCallback(cb);
		},

		mouseRelease : function (buttons, cb) {
			this._applet.mouseRelease(buttons);
			this._callCallback(cb);
		},

		mouseWheel : function (wheelAmt, cb) {
			this._applet.mouseWheel(wheelAmt);
			this._callCallback(cb);
		},

		keyPress : function (keycode, cb) {
			this._applet.keyPress(keycode);
			this._callCallback(cb);
		},

		keyRelease : function (keycode, cb) {
			this._applet.keyRelease(keycode);
			this._callCallback(cb);
		},
		...
		_screenCaptureCb : function (unused, args) {
			...
			this._applet.screenCapture(geometry.x, geometry.y, geometry.width, geometry.height, args.imageName);
			this.$callback(args.cb);
		},

		compareImages : function (imageName1, imageName2) {
			return this._applet.compareImages(imageName1, imageName2);
		},

		loadImage : function (url, imageName) {
			this._applet.loadImage(url, imageName);
		},

		saveImage : function (imageName, storePath) {
			this._applet.saveImage(imageName, storePath);
		},
		...
		getWin32Api : function () {
			return this._applet.getWin32Api();
		}
	}
});