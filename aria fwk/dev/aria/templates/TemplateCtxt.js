...
(function () {
	...
	var methodMapping = ["$refresh", "$getChild", "$getElementById", "$focus", "$hdim", "$vdim", "getContainerScroll",
			"setContainerScroll", "__$writeId", "__$processWidgetMarkup", "__$beginContainerWidget",
	...
	Aria.classDefinition({
		...
		$dependencies : ['aria.templates.Layout', 'aria.templates.CfgBeans', 'aria.utils.Array', 'aria.utils.Function',
				'aria.utils.Type', 'aria.templates.TemplateCtxtManager', 'aria.templates.ModuleCtrlFactory',
		...
		$destructor : function () {
			...
			if (this._cssClasses) {
				...
				if (this._globalCssDepsLoaded) {
					...
					aria.templates.CSSMgr.unloadWidgetDependencies('aria.templates.Template', ['aria.widgets.css.'
							+ aria.widgets.AriaSkinInterface.getSkinName() + '.Global']);
		...
		$prototype : {
			...
			__insertSection : function (section, skipInsertHTML) {
				...
				var params = this._cfg;
				var tpl = this._tpl;
				var domElt = !section.id ? params.tplDiv : aria.utils.Dom.getElementById(this.createId(section.id));
				if (domElt) {
					if (!skipInsertHTML) {
			...
			/**
			 * Create a global id from an id specified in a template. It add a template-specific suffix or prefix so
			...
			__$writeId : function (id) {
				...
				// templateCtxt)
				var genId = this._out.tplCtxt.createId(id);
			...
			$getChild : function (id, index) {
				var Dom = aria.utils.Dom, genId = this.createId(id), parent = Dom.getElementById(genId);
				if (parent) {
			...
			$focus : function (id) {
				...
				if (!focusSuccess) {
					var domElementId = this.createId(id);
			...
			getCSSClassNames : function (onlyCSSDep) {
				...
				if (!classes) {
					if (this._cfg.isRootTemplate) {
						...
						aria.templates.CSSMgr.loadWidgetDependencies('aria.templates.Template', ['aria.widgets.css.'
								+ aria.widgets.AriaSkinInterface.getSkinName() + '.Global']);
